...
class HelpCommands(commands.Cog):
    ...
    @staticmethod
    async def get_help_embed(channel):
        max_fields = 25
        embeds = []
        embed = Embed(title="Help", description="Here are the commands you can use:", color=0x33B0FF)

        for i, (command, description) in enumerate(HelpCommands.commands_dict.items()):
    ...
    async def handle_help_command(self, ctx):
        await HelpCommands.get_help_embed(ctx.channel)

def setup(bot):
    bot.add_cog(HelpCommands(bot))

async def get_help(channel):
    await HelpCommands.get_help_embed(channel)